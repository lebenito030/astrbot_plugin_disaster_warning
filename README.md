...
留空则根据下方开关决定行为                # 推送国内或包含国外
```

**国际事件开关**：

```text
地震白名单为空时是否包含国外事件: false  # 关闭=仅国内，开启=含国外
气象白名单为空时是否包含国外事件: false  # 关闭=仅国内，开启=含国外
```
...
- **智能过滤**：
  - 白名单不为空：只推送白名单中的省份（国外事件被过滤）
  - 白名单为空 + 开关关闭（默认）：只推送国内事件（国外事件被过滤）
  - 白名单为空 + 开关开启：推送所有事件（包含国外）
...
- 白名单仅对包含省份信息的预警有效
- 地震/海啸和气象预警使用不同的白名单
- 默认情况下（开关关闭），白名单为空时只推送国内事件
- 若需接收国外事件，请开启对应的"包含国外事件"开关
- 白名单不为空时，无法提取省份信息的事件将被过滤
